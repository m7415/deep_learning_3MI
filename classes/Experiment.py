        ...
        try:
            ...
            with open(self.csv_path, 'r') as file:
                ...
                if len(pd.read_csv(self.csv_path)) == 0:
                    ...
                    last_experiment_number = pd.read_csv(self.csv_path).iloc[-1]['Experiment number']
            ...
            with open(self.csv_path, 'a+', newline='') as file:
                writer = csv.writer(file)
                writer.writerow([number, self.name, datetime.datetime.now(), optimizer_name, self.learning_rate, loss_name, self.input_shape, self.output_shape, self.filters, self.dropout, self.epochs, self.batch_size, self.test_ssim, self.test_psnr, self.test_mse])
                file.close()
